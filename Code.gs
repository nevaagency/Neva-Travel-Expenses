...
function handleRequest(e) {
  ...
  try {
    ...
    switch (action) {
      ...
      case 'getSaldoFerias':     result = getSaldoFerias(params.funcionaria, params.ano); break;
      ...
      case 'addFerias':          result = addFerias(JSON.parse(e.postData?.contents || '{}')); break;
      ...
      case 'deleteFerias':       result = deleteRow('Ferias', params.id); break;
...
function getEntradas(funcionaria, ano, mes) {
  ...
  const horasGozadas = getSheetRows('HorasGozadas')
    .filter(r => r.Funcionaria === funcionaria && String(r.Data).startsWith(ano + '-' + mesNumero(mes)));
  // Férias: filtrar pelo ano inteiro (não por mês)
  let ferias = [];
  try {
    ferias = getSheetRows('Ferias')
      .filter(r => r.Funcionaria === funcionaria && String(r.Ano) === String(ano));
  } catch(e) {}
  return { deslocacoes, parques, horasExtra, horasGozadas, ferias };
}
...
function getTotalDiasFerias(funcionaria, ano) {
  try {
    const config = getSheetRows('FeriasConfig')
      .find(r => r.Funcionaria === funcionaria && String(r.Ano) === String(ano));
    if (config && config.TotalDias) return Number(config.TotalDias);
  } catch(e) {}
  return 22; // default
}

function getSaldoFerias(funcionaria, ano) {
  const totalDias = getTotalDiasFerias(funcionaria, ano);
  let rows = [];
  try {
    rows = getSheetRows('Ferias')
      .filter(r => r.Funcionaria === funcionaria && String(r.Ano) === String(ano));
  } catch(e) {}
  const totalGozados = rows.reduce((s, r) => s + Number(r.Dias || 0), 0);
  return { total: totalDias, gozados: totalGozados, saldo: totalDias - totalGozados };
}

function addFerias(data) {
  const id = Date.now().toString();
  getSheet('Ferias').appendRow([
    id, data.funcionaria, data.ano, data.data,
    data.dias, new Date().toISOString()
  ]);
