...
async function generatePDF(payload) {
  ...
  // Normalize KM entries: always Porto as origem
  const kmsRaw = (payload.kms || []).map(r => {
    const entry = { ...r };
    const orig = (entry.origem || 'Porto').trim().toLowerCase();
    const dest = (entry.destino || '').trim().toLowerCase();
    if (orig !== 'porto') {
      if (dest === 'porto') {
        // reverse trip: swap (e.g. Braga->Porto becomes Porto->Braga)
        entry.destino = entry.origem;
      } else {
        // neither end is Porto: find city at equivalent distance
